...
export const SubsidiesUserAssignmentPage = ({ title }: { title: string }) => {
  ...
  useEffect(() => {
    console.log("UCITAO SUBVENCIJEJ");
  }, []);
  ...
  const handleCreate: MRT_TableOptions<SubsidyAssignment>["onCreatingRowSave"] = async ({ values, table }) => {
    try {
      ...
      const body = {
        korisnik_id: userID,
        subvencija_id: Number(parts[0].trim()),
        status: values["status"],
      };

      setIsSaving(true);
      const { success, data }: { success: boolean; data: SubsidyAssignment } = await fetchAPI(
        "/api/subsidies-user-assignment/assign",
  ...
  const handleUpdate: MRT_TableOptions<SubsidyAssignment>["onEditingRowSave"] = async ({ values, row, table }) => {
    try {
      ...
      const { success, data } = await fetchAPI("/api/subsidies-user-assignment/reassign", { method: "POST", data: body });
      if (success) {
  ...
  const handleDelete = async (row: MRT_Row<SubsidyAssignment>) => {
    if (window.confirm("Da li potvrdjujete brisanje?")) {
      try {
        setIsSaving(true);
        const { success }: { success: boolean; data: SubsidyAssignment } = await fetchAPI(
          "/api/subsidies-user-assignment/remove",
          { method: "POST", data: { id: row.original.id } }
        );
      ...
      },
      {
        ...
        Edit: ({ row, column }) => {
          ...
          return (
            <SearchList
              label="Subvencija"
              endpoint={`/api/subsidies/getSubsidiesForSL`}
              value={value}
              multiple={false}
  ...
  async function handleUserAccountChange(newValue: string) {
    try {
      ...
      const data = await fetchAPI(`/api/subsidies-user-assignment/assigned/${userId}`, { method: "POST" });
      setIsFetching(false);
      setData(data || []);
