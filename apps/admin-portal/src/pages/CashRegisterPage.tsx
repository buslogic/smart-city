...
export const CashRegisterPage = ({ title }: { title: string }) => {
    const { fetchData, createRow, deleteRow, updateRow, isCreating, cashRegister, columns, isDeleting, isFetching, isUpdating } = useCashRegister();

    useEffect(() => {
        fetchData();
    }, []);

    const handleCreate: MRT_TableOptions<CashRegister>['onCreatingRowSave'] = async ({ values, table }) => {
        try {
    ...
    const handleUpdate: MRT_TableOptions<CashRegister>['onEditingRowSave'] = async ({ values, row, table }) => {
        try {
    ...
    const handleDelete = async (row: MRT_Row<CashRegister>) => {
    ...
    const table = useMaterialReactTable({
        ...
        renderEditRowDialogContent: ({ table, row, internalEditComponents }) => {
            const reader = cashRegister.find((reader) => reader.id === row.original.id);
            if (!reader) {
