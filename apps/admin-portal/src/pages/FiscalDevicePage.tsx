...
import useFiscalDevice from '@/hooks/useFiscalRegister';
...
export const FiscalDevicePage = ({ title }: { title: string }) => {
    const { fetchData, createRow, deleteRow, updateRow, isCreating, fiscalDevice, columns, isDeleting, isFetching, isUpdating } = useFiscalDevice();

    useEffect(() => {
        fetchData();
    }, []);

    const handleCreate: MRT_TableOptions<FiscalDevice>['onCreatingRowSave'] = async ({ values, table }) => {
        try {
    ...
    const handleUpdate: MRT_TableOptions<FiscalDevice>['onEditingRowSave'] = async ({ values, row, table }) => {
        try {
    ...
    const handleDelete = async (row: MRT_Row<FiscalDevice>) => {
    ...
    const table = useMaterialReactTable({
        ...
        renderEditRowDialogContent: ({ table, row, internalEditComponents }) => {
            const reader = fiscalDevice.find((reader) => reader.id === row.original.id);
            if (!reader) {
...
export default FiscalDevicePage;
