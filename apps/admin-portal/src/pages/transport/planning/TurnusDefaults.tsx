...
const TurnusDefaults: React.FC = () => {
  ...
  const [selectedTurnusName, setSelectedTurnusName] = useState<string | undefined>();
  ...
  useEffect(() => {
    applyFilters();
  }, [defaults, selectedDriver, showActiveOnly, selectedTurnusName]);
  ...
  const applyFilters = () => {
    ...
    if (selectedTurnusName) {
      filtered = filtered.filter((d) => d.turnusName === selectedTurnusName);
    }
      ...
      <Card>
        ...
        <Card size="small" className="mb-4">
          <Row gutter={16}>
            ...
            <Col span={8}>
              <Select
                placeholder="Filtriraj po turnusu"
                allowClear
                showSearch
                style={{ width: '100%' }}
                value={selectedTurnusName}
                onChange={setSelectedTurnusName}
              >
                {Array.from(new Set(defaults.map((d) => d.turnusName)))
                  .sort((a, b) => a.localeCompare(b))
                  .map((turnusName) => (
                    <Option key={turnusName} value={turnusName}>
                      {turnusName}
                    </Option>
                  ))}
              </Select>
