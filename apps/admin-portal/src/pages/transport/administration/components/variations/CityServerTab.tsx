...
const CityServerTab: React.FC = () => {
  ...
  const loadData = async (page = 1, pageSize = 50) => {
    setLoading(true);
    try {
      const result = await priceVariationsService.getAllCity(page, pageSize);
      setData(result.data);
      setPagination({
        current: result.page,
      ...
      console.error('❌ Greška pri učitavanju varijacija (Gradski):', error);
  ...
  const handleSync = () => {
    modal.confirm({
      ...
      onOk: async () => {
        setSyncing(true);
        try {
          const result = await priceVariationsService.syncFromCity();
    ...
    <div>
      <Card className="mb-4">
        <Row justify="space-between" align="middle">
          <Col>
            <Space>
              <GlobalOutlined style={{ fontSize: 24, color: '#fa8c16' }} />
              <div>
                <Title level={4} style={{ margin: 0 }}>Gradski Server</Title>
                <Text type="secondary">Varijacije na Gradskom serveru (READ-ONLY)</Text>
              </div>
            </Space>
          </Col>
          <Col>
            <Space>
              ...
              {hasPermission('transport.administration.variations.city:sync') && (
                <Button
                  type="primary"
