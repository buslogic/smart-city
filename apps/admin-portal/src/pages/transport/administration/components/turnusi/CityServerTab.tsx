...
const CityServerTab: React.FC = () => {
  ...
  const loadGroups = async () => {
    setLoading(true);
    try {
      const data = await turnusiService.getAllGroupsCity();
      setGroups(data);
    } catch (error: any) {
  ...
  const handleSync = () => {
    ...
    modal.confirm({
      ...
      onOk: async () => {
        ...
        try {
          const response = await turnusiService.syncFromCityAsync(selectedGroupId);
      ...
      <Card className="mb-4">
            ...
            <Space>
              ...
              <div>
                <Title level={4} style={{ margin: 0 }}>
                  City Server - Changes Codes Tours
                </Title>
                <Text type="secondary">
        ...
        <Row gutter={16} style={{ marginTop: 16 }}>
          ...
          <Col span={12} style={{ display: 'flex', alignItems: 'flex-end' }}>
            {hasPermission('transport.administration.turnusi.city:sync') && (
              <Button
                type="primary"
