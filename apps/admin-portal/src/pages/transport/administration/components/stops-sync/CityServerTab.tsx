...
const CityServerTab: React.FC = () => {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const stops = await stopsSyncService.getAllCity();
      setData(stops);
    } catch (error: any) {
      console.error('Greška pri učitavanju stajališta (Gradski):', error);
  ...
  const handleSync = () => {
    modal.confirm({
      ...
      onOk: async () => {
        setSyncing(true);
        try {
          const result = await stopsSyncService.syncFromCity();
          ...
          <Col>
            <Space>
              ...
              <div>
                ...
                <Text type="secondary">Stajališta na Gradskom serveru (READ-ONLY)</Text>
              </div>
            </Space>
          </Col>
          <Col>
            <Space>
              ...
              {hasPermission('transport.administration.stops_sync.city:sync') && (
                <Button
                  type="primary"
