...
import { priceListGroupsService } from '../../../../../services/priceListGroups.service';
...
const TicketingServerTab: React.FC = () => {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const groups = await priceListGroupsService.getAllTicketing();
      setData(groups);
    } catch (error: any) {
      console.error('Greška pri učitavanju grupa cenovnika (Tiketing):', error);
  ...
  const handleSync = () => {
    modal.confirm({
      ...
      onOk: async () => {
        setSyncing(true);
        try {
          const result = await priceListGroupsService.syncFromTicketing();
          ...
          <Col>
            <Space>
              ...
              <div>
                ...
                <Text type="secondary">Grupe cenovnika na Tiketing serveru (READ-ONLY)</Text>
              </div>
            </Space>
          </Col>
          <Col>
            <Space>
              ...
              {hasPermission('transport.administration.price_list_groups.ticketing:sync') && (
                <Button
                  type="primary"
