...
const CityServerTab: React.FC = () => {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const groups = await priceListGroupsService.getAllCity();
      setData(groups);
    } catch (error: any) {
      console.error('Greška pri učitavanju grupa cenovnika (Gradski):', error);
  ...
  const handleSync = () => {
    modal.confirm({
      ...
      onOk: async () => {
        setSyncing(true);
        try {
          const result = await priceListGroupsService.syncFromCity();
          ...
          <Col>
            <Space>
              ...
              <div>
                ...
                <Text type="secondary">Grupe cenovnika na Gradskom serveru (READ-ONLY)</Text>
              </div>
            </Space>
          </Col>
          <Col>
            <Space>
              ...
              {hasPermission('transport.administration.price_list_groups.city:sync') && (
                <Button
                  type="primary"
                  icon={<SyncOutlined />}
                  onClick={handleSync}
