...
export const ReadingsPage = ({ title }: { title: string }) => {
    const { fetchData, createRow, deleteRow, updateRow, isCreating, readings, columns, isDeleting, isFetching, isUpdating } = useReadings();

    useEffect(() => {
        fetchData();
    }, []);

    const handleCreate: MRT_TableOptions<Readings>['onCreatingRowSave'] = async ({ values, table }) => {
        try {
    ...
    const handleUpdate: MRT_TableOptions<Readings>['onEditingRowSave'] = async ({ values, row, table }) => {
        try {
    ...
    const handleDelete = async (row: MRT_Row<Readings>) => {
    ...
    const table = useMaterialReactTable({
        ...
        renderCreateRowDialogContent: ({ row, table, internalEditComponents }) => {
            if (!row._valuesCache['datum']) {
                row._valuesCache['datum'] = dayjs().format('YYYY-MM-DD');
            }
