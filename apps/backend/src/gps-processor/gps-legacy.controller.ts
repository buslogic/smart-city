...
export class GpsLegacyController {
  ...
  constructor(private readonly gpsProcessorService: GpsProcessorService) {
    this.apiKey = process.env.GPS_LEGACY_API_KEY || 'gps-legacy-key-2025-secure';
  }
  ...
  @ApiOperation({ 
    ...
    description: 'Endpoint za prijem batch GPS podataka sa legacy PHP sistema. Zahteva API key autentifikaciju.'
  })
  @ApiHeader({
    name: 'x-api-key',
  ...
  async ingestGpsData(
    ...
    if (!apiKey || apiKey !== this.apiKey) {
      this.logger.warn(`Unauthorized access attempt with key: ${apiKey?.substring(0, 10)}...`);
      throw new UnauthorizedException('Invalid API key');
    }

    // Validacija podataka
    if (!data || !data.points || !Array.isArray(data.points)) {
      throw new BadRequestException('Invalid data format - points array required');
    }
    ...
    if (data.points.length > 500) {
      throw new BadRequestException('Too many points in single batch (max 500)');
    }

    // Log prijem podataka
    this.logger.log(`Received ${data.points.length} GPS points from legacy system`);

    try {
      // Procesiranje podataka kroz postojeći servis
      const result = await this.gpsProcessorService.processLegacyBatch(data.points);
      
      this.logger.log(`Successfully processed ${result.processed} points, failed: ${result.failed}`);
