...
async function testMultipleVehicles() {
  ...
  console.log(
    '🚀 Test Worker Pool importa za vozila:',
    garageNumbers.join(', '),
  );
  ...
  try {
    ...
    vehicles.forEach((v) =>
      console.log(`   - ${v.garageNumber} (ID: ${v.id})`),
    );
    ...
    result.forEach((workerResult, vehicleId) => {
      ...
      console.log(
        `   Procesirano: ${workerResult.processedRecords.toLocaleString()} GPS tačaka`,
      );
    ...
    console.log(
      `   Brzina: ${(totalProcessed / duration).toFixed(0)} tačaka/sekund`,
    );
  } catch (error) {
