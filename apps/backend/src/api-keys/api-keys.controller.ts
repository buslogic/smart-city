...
export class ApiKeysController {
  ...
  @ApiOperation({
    ...
    description:
      'Generiše novi sigurni API ključ. NAPOMENA: Ključ se prikazuje samo jednom!',
  })
  @ApiResponse({
    status: 201,
  ...
  async findAll(
    ...
    const isAdmin =
      req.user.permissions?.includes('api_keys:manage') ||
      req.user.roles?.includes('SUPER_ADMIN');

    return this.apiKeysService.findAll(isAdmin ? filterUserId : req.user.id);
  }
  ...
  @ApiOperation({
    ...
    description:
      'Ažurira postojeći API ključ (ne menja sam ključ, već metadata)',
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiResponse({
    status: 400,
    description: 'Ne može se ažurirati revokovan ključ',
  })
  async update(
    @Param('id', ParseIntPipe) id: number,
  ...
  @ApiOperation({
    ...
    description:
      'POTPUNO briše API ključ iz baze. Koristi revoke umesto brisanja!',
  })
  ...
  async remove(
    ...
    return {
      message: 'Brisanje API ključa nije implementirano. Koristite revoke.',
    };
  }
}
