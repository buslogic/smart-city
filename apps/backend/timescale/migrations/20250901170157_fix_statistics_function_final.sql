-- migrate:up
-- Eksplicitno briši staru funkciju i kreiraj novu sa ispravnim enum vrednostima

-- Prvo obriši funkciju ako postoji
DROP FUNCTION IF EXISTS get_vehicle_driving_statistics CASCADE;

-- Kreiraj funkciju sa ispravnim enum vrednostima
...
BEGIN
    ...
    WITH event_stats AS (
        SELECT 
            ...
            -- Koristi ispravne enum vrednosti: harsh_acceleration i harsh_braking
...
'Vraća statistiku agresivne vožnje za vozilo u datom periodu - FINALNA VERZIJA sa ispravnim enum vrednostima';

-- Verifikacija da funkcija koristi ispravne enum vrednosti
DO $$
BEGIN
    RAISE NOTICE '✅ Funkcija get_vehicle_driving_statistics je ažurirana sa ispravnim enum vrednostima (harsh_acceleration, harsh_braking)';
END $$;
