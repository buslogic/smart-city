-- migrate:up
-- Optimizovana paralelna migracija za timezone fix
-- Eliminiše OFFSET pristup i omogućava paralelno procesiranje

-- ============================================
-- 1. ISPRAVKA POSTOJEĆE migrate_single_day PROCEDURE
-- ============================================
-- Prvo proveri da li postoji stara funkcija i obriši je
DO $$
BEGIN
    -- Obriši funkciju ako postoji (različite signature)
    DROP FUNCTION IF EXISTS migrate_single_day(DATE) CASCADE;
    DROP FUNCTION IF EXISTS migrate_single_day(DATE, INTEGER) CASCADE;

    -- Obriši proceduru ako postoji
    DROP PROCEDURE IF EXISTS migrate_single_day(DATE, BIGINT, INTERVAL, INTEGER) CASCADE;
EXCEPTION
...
BEGIN
    ...
    LOOP
        -- KLJUČNA OPTIMIZACIJA: WHERE time > last_processed umesto OFFSET
...
-- ============================================
-- 2. NOVA PROCEDURA ZA PARALELNU MIGRACIJU
-- ============================================
-- Procedura za migraciju vremenskog opsega
CREATE OR REPLACE PROCEDURE migrate_time_range(
...
BEGIN
    ...
    LOOP
        ...
        ELSE
            ...
            IF v_total_processed % 10000000 = 0 THEN
                RAISE NOTICE '[%] Processed % records so far...',
                    COALESCE(p_range_name, 'RANGE'), v_total_processed;
            END IF;
        END IF;
    END LOOP;
    ...
    RAISE NOTICE '[%] Completed: % records in %',
        COALESCE(p_range_name, 'RANGE'), out_records_migrated, out_duration;

    -- Log u migration_log tabelu
...
-- ============================================
-- 3. HELPER FUNKCIJA ZA PODELU DANA
-- ============================================
CREATE OR REPLACE FUNCTION split_day_into_ranges(
    p_date DATE,
...
-- ============================================
-- 4. FUNKCIJA ZA PRAĆENJE NAPRETKA
-- ============================================
CREATE OR REPLACE FUNCTION check_range_progress(p_date DATE)
RETURNS TABLE (
    range_name TEXT,
    start_time TIMESTAMP,
    end_time TIMESTAMP,
...
-- migrate:down
-- Rollback: obriši nove funkcije i vrati staru verziju
DROP FUNCTION IF EXISTS check_range_progress(DATE);
DROP FUNCTION IF EXISTS split_day_into_ranges(DATE, INTEGER);
DROP PROCEDURE IF EXISTS migrate_time_range(TIMESTAMP, TIMESTAMP, TEXT, BIGINT, INTERVAL, INTEGER);
