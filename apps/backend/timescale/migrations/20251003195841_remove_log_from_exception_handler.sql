-- migrate:up
-- ==============================================================================
-- FIX: Ukloni INSERT log iz exception handler-a
-- ==============================================================================
-- Problem: Exception handler pokušava da logiše sa batch_id koji je rollback-ovan
-- Rešenje: Samo update status ako postoji, bez logiranja
-- ==============================================================================
...
BEGIN
    ...
    -- Log u processing_log (samo za uspešan slučaj)
...
EXCEPTION WHEN OTHERS THEN
    ...
    -- Pokušaj update status-a (možda neće raditi zbog rollback-a)
    BEGIN
        UPDATE gps_processing_status
        SET
            status = 'failed',
        ...
        WHERE id = v_batch_id;
    EXCEPTION WHEN OTHERS THEN
        -- Ignoriši ako ne može da update-uje
        NULL;
    END;

    -- NE pokušavaj INSERT u log tabelu jer će biti rollback-ovan
    -- Samo RAISE ponovo originalnu grešku
    RAISE EXCEPTION 'Batch processing failed: %', v_error_msg;
END;
$$ LANGUAGE plpgsql;

COMMENT ON FUNCTION process_gps_batch IS
'Glavna funkcija za batch procesiranje GPS podataka sa LAG() kalkulacijama.
FIX: Exception handler ne pokušava INSERT u log tabelu zbog transakcijskih problema.';
