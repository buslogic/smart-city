...
# =============================================================================
# Fast Direct GPS Import to TimescaleDB - Docker Version
# =============================================================================
# Ova skripta direktno importuje GPS podatke u TimescaleDB koristeći COPY
# Radi sa Docker kontejnerima koristeći stdin za COPY komandu
# =============================================================================

set -e  # Exit on error

# Parameters
DUMP_FILE="${1:-/home/kocev/smart-city/scripts/P93598gps_export.sql.gz}"
GARAGE_NO="${2:-P93598}"
BATCH_SIZE="${3:-100000}"  # 100K zapisa po batch-u

# MySQL container za proveru vozila
MYSQL_CONTAINER="smartcity-mysql-local"
MYSQL_USER="root"
MYSQL_PASS="root_password"
MYSQL_DB="smartcity_dev"

# TimescaleDB konfiguracija
TS_CONTAINER="smartcity-timescale-local"
TS_DB="smartcity_gps"
TS_USER="smartcity_ts"
TS_PASS="smartcity_ts_pass"

echo "================================================"
echo "⚡ Fast GPS Import - Docker Version"
echo "================================================"
...
echo ""
echo "Step 3: Direktan import u TimescaleDB..."
...
for ((i=0; i<$TOTAL_BATCHES; i++)); do
  ...
  cat $TEMP_DIR/batch_$i.csv | PGPASSWORD=$TS_PASS docker exec -i $TS_CONTAINER psql -U $TS_USER -d $TS_DB -c "
    BEGIN;
...
TIME_RANGE=$(PGPASSWORD=$TS_PASS docker exec $TS_CONTAINER psql -U $TS_USER -d $TS_DB -t -c "
  SELECT 
...
if [ ! -z "$START_DATE" ] && [ ! -z "$END_DATE" ]; then
  ...
  while [ "$current_date" \< "$END_DATE" ] || [ "$current_date" = "$END_DATE" ]; do
    ...
    PGPASSWORD=$TS_PASS docker exec $TS_CONTAINER psql -U $TS_USER -d $TS_DB -t -c "
  ...
  while [ "$current_month" \< "$END_DATE" ] || [ "$current_month" = "$END_DATE" ]; do
    ...
    PGPASSWORD=$TS_PASS docker exec $TS_CONTAINER psql -U $TS_USER -d $TS_DB -c "
...
IMPORTED=$(PGPASSWORD=$TS_PASS docker exec $TS_CONTAINER psql -U $TS_USER -d $TS_DB -t -c "
  SELECT COUNT(*) 
  FROM gps_data 
  WHERE vehicle_id = $VEHICLE_ID 
    AND data_source = 'historical_fast_import';
...
EVENTS=$(PGPASSWORD=$TS_PASS docker exec $TS_CONTAINER psql -U $TS_USER -d $TS_DB -t -c "
  SELECT COUNT(*) 
  FROM driving_events 
  WHERE vehicle_id = $VEHICLE_ID 
