...
# Analiziraj komandu koja se pokušava izvršiti
COMMAND="$1"
...
for pattern in "${FORBIDDEN_PATTERNS[@]}"; do
    if echo "$COMMAND" | grep -iE "$pattern" > /dev/null 2>&1; then
        ...
        if [ "$is_allowed" = false ]; then
            echo "❌ BLOKIRAN: Direktan upis u bazu podataka nije dozvoljen!"
            echo ""
            echo "⚠️  Pokušali ste da izvršite: $COMMAND"
            echo ""
            echo "✅ ISPRAVNO: Koristite Prisma migracije za izmene baze:"
            echo "   • cd apps/backend && npx prisma migrate dev --name naziv_migracije"
            echo "   • cd apps/backend && npx prisma db push (samo za development)"
            echo ""
            echo "📚 Za TimescaleDB koristite dbmate migracije:"
            echo "   • cd apps/backend/timescale"
            echo "   • dbmate new naziv_migracije"
            echo "   • dbmate up"
            echo ""
            echo "ℹ️  Dozvoljene su samo SELECT, SHOW i DESCRIBE komande za čitanje podataka."
            exit 1
        fi
    fi
done
    ...
    if [ -f "$SQL_FILE" ]; then
        if grep -iE "INSERT|UPDATE|DELETE|CREATE|ALTER|DROP|TRUNCATE" "$SQL_FILE" > /dev/null 2>&1; then
            if ! grep -iE "prisma|migration|dbmate" "$SQL_FILE" > /dev/null 2>&1; then
                echo "❌ BLOKIRAN: SQL fajl '$SQL_FILE' sadrži direktne upise u bazu!"
                echo ""
                echo "✅ Koristite Prisma ili dbmate migracije umesto direktnih SQL skripti."
                exit 1
            fi
        fi
    fi
